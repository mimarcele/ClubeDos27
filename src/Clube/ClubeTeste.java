...
public class ClubeTeste {
    public static void main(String[] args) {
        ...
        Predicate<Clube> idadeMort = i -> i.idadeMorte == 27;

        Predicate<Clube> generoMusical = g -> g.generoMusical.equals("Blues-Rock");

        System.out.println("Os cantores de Blues-Rock do clube dos 27 são:");
        Function<Clube, String> resultado = r -> r.nome;

        clube.stream().filter(idadeMort).filter(generoMusical).map(resultado).forEach(System.out::println);
    }
}
